...
function generateLargeTestFile(lines) {
    const components = [
        "Button",
        "Input",
        "Container",
    ...
    for (let i = 0; i < Math.min(components.length, Math.floor(lines / 20)); i++) {
...
\`;\n\n`;
    ...
    const usagesPerComponent = Math.floor(remainingLines / (components.length * 3));
...
function generateFileWithoutEmotion(lines) {
...
describe("Performance Tests", () => {
    ...
    describe("Large file performance", () => {
        it("should process medium files quickly (1000 lines)", async () => {
            ...
            console.log(`Medium file (1000 lines) analysis time: ${analysisTime.toFixed(2)}ms`);
        ...
        it("should process large files reasonably quickly (5000 lines)", async () => {
            ...
            const end = performance.now();
            const analysisTime = end - start;
            console.log(`Large file (5000 lines) analysis time: ${analysisTime.toFixed(2)}ms`);
        ...
        it("should process very large files within acceptable limits (10000 lines)", async () => {
            ...
            const end = performance.now();
            const analysisTime = end - start;
            console.log(`Very large file (10000 lines) analysis time: ${analysisTime.toFixed(2)}ms`);
    ...
    describe("Early exit performance", () => {
        it("should exit very quickly for files without emotion imports", async () => {
            ...
            const end = performance.now();
            const analysisTime = end - start;
            console.log(`File without emotion (5000 lines) analysis time: ${analysisTime.toFixed(2)}ms`);
      ...
      `;
            ...
            const end = performance.now();
            const analysisTime = end - start;
            console.log(`File with emotion imports but no components analysis time: ${analysisTime.toFixed(2)}ms`);
    ...
    describe("Cache performance", () => {
        it("should use cache effectively for repeated analysis", async () => {
            ...
            console.log(`Second analysis time (cached): ${secondAnalysisTime.toFixed(2)}ms`);
        ...
        `;
            ...
            console.log(`Multiple files (10) analysis time: ${totalTime.toFixed(2)}ms`);
            ...
            console.log(`Multiple files (10) cached analysis time: ${cachedTime.toFixed(2)}ms`);
    ...
    describe("Stress tests", () => {
        it("should handle files with many styled components", async () => {
            ...
            for (let i = 0; i < 100; i++) {
                code += `const Component${i} = styled.div\`color: hsl(${i * 3.6}, 50%, 50%);\`;\n`;
            }
            code += "\nconst App = () => (\n  <div>\n";
            for (let i = 0; i < 100; i++) {
            ...
            const end = performance.now();
            const analysisTime = end - start;
            console.log(`Many components (100) analysis time: ${analysisTime.toFixed(2)}ms`);
